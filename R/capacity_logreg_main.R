...
capacity_logreg_main<-function(dataRaw, signal="signal", response="response",side_variables=NULL,
  ...
  if (graphs){
    output$logGraphs=try(output_graphs(data=dataRaw,signal=signal,response=response,side_variables=side_variables,cc_output=output,
                                path=output_path,height=plot_height,width=plot_width),
        silent=FALSE)
