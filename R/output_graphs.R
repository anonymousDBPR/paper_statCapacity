...
#' 
#' @keywords internal
output_graphs<-function(data,signal,response,side_variables,cc_output,
                        path,height=4,width=6){
  
  data=aux_signal_transform(data,signal)
  
  plot_io=try(output_graph_io(data,signal,response,path,height=height,width=width),silent=TRUE)
  plot_box=try(output_graph_boxplots(data,signal,response,path,height=height,width=width),silent=FALSE)
  plot_violin=try(output_graph_violinMean(data,signal,response,path,height=height,width=width),silent=FALSE)
  plot_hist=try(output_graph_histograms(data,signal,response,path,height=height,width=width),silent=FALSE)
  plot_boxSideVar=try(output_graph_boxplotsSideVar(data,signal,side_variables,path,height=height,width=width) ,silent=FALSE)
  plot_capacity=try(output_graph_capacity(cc_output,path,height=height,width=width),silent=FALSE)
  plot_density=try(output_graph_densities(data,signal,response,path,height=height,width=width),silent=FALSE)
  ...
  try(ggplot2::ggsave(plot_main,file=paste(path,'MainPlot_big.pdf',sep=""),height=5*height,width=2*width,limitsize = FALSE),silent=FALSE)
  ...
  if (is.null(cc_output$testing)){
    ...
    try(ggplot2::ggsave(plot_main_simp,file=paste(path,'MainPlot.pdf',sep=""),height=2*height,width=2*width,limitsize = FALSE),silent=FALSE)
  } else {
    plot_main_simp=try(gridExtra::grid.arrange(plot_violin,plot_box,plot_capacity,
                                          layout_matrix=rbind(c(1,2),
                                                              c(3,3),
    ...
    try(ggplot2::ggsave(plot_main_simp,file=paste(path,'MainPlot.pdf',sep=""),height=3*height,width=2*width,limitsize = FALSE),silent=FALSE)
  }
...
#' @rdname output_graphs
output_graph_io<-function(data,signal,response,path,height,width){
  ...
  if (any(data_colnames %in% (signalNUM))){
    ...
    plot<-ggplot2::ggplot(data=dataPlot,ggplot2::aes_string(x=signal,y="value"))+ggplot2::stat_summary(fun.y=mean,geom="point",size=2.35)+
      ggplot2::facet_grid(.~variable)+
      ggplot2::scale_y_continuous(paste("Response ",sep="") )+ggplot2::scale_x_discrete(paste("Signal - ",signal,sep="") )+
      ggplot2::ggtitle("Input-Output Relation")+
      theme_publ(version=2)
    
    plot_col<-ggplot2::ggplot(data=dataPlot,ggplot2::aes_string(x=signal,y="value"),ggplot2::aes(colour=factor(variable)))+ggplot2::stat_summary(fun.y=mean,geom="point",size=2.35)+
      ggplot2::scale_y_continuous(paste("Response ",sep="") )+ggplot2::scale_x_discrete(paste("Signal - ",signal,sep="") )+
      ggplot2::ggtitle("Input-Output Relation")+
      theme_publ(version=2)
  }
...
#' @rdname output_graphs
output_graph_boxplots<-function(data,signal,response,path,height=4,width=6){
  ...
  plot<-ggplot2::ggplot(data=dataPlot,ggplot2::aes_string(x=signal,y="value") ) + 
    ...
    ggplot2::facet_grid(.~variable)+
    ggplot2::scale_y_continuous(name=paste("Response - ",sep="") )+
    ggplot2::scale_x_discrete(name=paste("Signal - ",signal,sep="") )+
    ggplot2::ggtitle("Output distributions")+
    theme_publ(version=2)
...
#' @rdname output_graphs
output_graph_violinMean<-function(data,signal,response,path,height=4,width=6){
  ...
  if (any(data_colnames %in% (signalNUM))){
    ...
    plot<-ggplot2::ggplot(data=dataPlot,ggplot2::aes_string(x=signalNUM,y="value"))+ggplot2::geom_violin(ggplot2::aes_string(group=signalNUM),scale="width")+
      ggplot2::stat_summary(fun.y=mean,geom="line",size=1.35)+ggplot2::stat_summary(fun.y=mean,geom="point",size=2.35)+
      ggplot2::facet_grid(variable~.)+
      ggplot2::scale_y_continuous(paste("Response ",sep="") )+ggplot2::scale_x_continuous(paste("Signal - ",signal,sep=""),limits=c(0,maxSignal*1.05) )+
      ggplot2::ggtitle("Input-Output Relation")+
      theme_publ(version=2)
  ...
  ggplot2:: ggsave(plot,file=paste(path,'violin_mean.pdf',sep=""),height=height,width=min(49.5,response_length*width))
...
#' @rdname output_graphs
output_graph_histograms<-function(data,signal,response,path,height=4,width=6){
  ...
  plot<-plotTemp + ggplot2::geom_text(data=tmp_data,ggplot2::aes(x=x,y=y,label=lab),vjust=1)+    
    ...
    ggplot2::ggtitle("Output histograms")+
    theme_publ(version=2)
...
#' @rdname output_graphs
output_graph_boxplotsSideVar<-function(data,signal,side_variables,path,height=4,width=6){
  ...
  if (is.null(side_variables)) {
    ...
    plot<-ggplot2::ggplot(data=dataMelt,ggplot2::aes_string(x=signal,y="value") ) + ggplot2::geom_boxplot()+ ggplot2::facet_grid(variable~.) +
      ggplot2::scale_y_continuous("side variable")+ggplot2::scale_x_discrete(paste("Signal - ",signal,sep="") )+
      ggplot2::ggtitle("Side variables distributions")+
      theme_publ(version=2)
...
#' @rdname output_graphs
output_graph_capacity<-function(cc_output,path,height=4,width=6){
  
  if (is.null(cc_output$testing)){
    
    plot<-ggplot2::ggplot(data=data.frame(x=cc_output$cc,y=0),ggplot2::aes(x=x,y=y) ) + 
      ggplot2::geom_point(size=4,shape=15) + 
      ggplot2::geom_text(ggplot2::aes(label=round(x,digits=3),y=0.025),size=10)+
    ...
    plot1<-ggplot2::ggplot(data=data.frame(x=cc_output$cc,y=0),ggplot2::aes(x=x,y=y) ) + 
      ...
      ggplot2::geom_text(data=data.frame(x=c(boot_mean-boot_sd,boot_mean+boot_sd),y=c(0,0)),ggplot2::aes(label=round(x,digits=3),y=-0.025),size=8, colour="red")+
    ...
    plot=gridExtra::grid.arrange(plot1,plot2,plot3,plot4,plot3b,plot5,plot6,plot7,plot8,plot9,plot10,plot11,plot12,
                      layout_matrix=rbind(c(1,1),c(2,6),c(2,7),c(3,8),c(3,9),c(4,10),c(4,11),c(5,12),c(5,13)))
...
#' @rdname output_graphs
output_graph_densities<-function(data,signal,response,path,height=4,width=6){
  ...
  plot<-ggplot2::ggplot(data=dataPlot,ggplot2::aes_string(x="value",fill=signal,colour=signal)) + ggplot2::geom_density(alpha = 0.3)+
    ...
    ggplot2::ggtitle("Output densities")+
    theme_publ(version=2)+ggplot2::theme(legend.position = "right")
  
  ggplot2::ggsave(plot,file=paste(path,'densitites.pdf',sep=""),height=height,width=width)
